...
object NaiveEstimator extends org.apache.spark.ml.Predictor {
  ...
  final val timeCol = new Param[String](
    this,
    "time_index",
    "The column with the date-time values"
  )
  final val inputCol = new Param[String](this, "y_input", "The input column")

  final val forecastCol =
    new Param[String](this, "y_forecast", "The output column")
  ...
  override def transformSchema(schema: StructType): StructType = {
    ...
    if (field.dataType != IntegerType || field.dataType != LongType) {
      throw new Exception(
        s"Time index data type ${field.dataType} did not match input type DoubleType"
      )
    }
    ...
    if (inputField.dataType != DoubleType) {
      throw new Exception(
        s"Input data type ${inputField.dataType} did not match input type DoubleType"
      )
    }

    // add output column metadata:
    var metaBuild: MetadataBuilder = new MetadataBuilder()
      .putString("Description", "Naiive forecast value": String);
...
class NaiveForecastModel(override val uid: String)
    extends PredictionModel[Vector[Double], NaiveForecastModel]
